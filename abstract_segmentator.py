...
class AbstractSegmentatorClass(metaclass=abc.ABCMeta):
    ...
    def __init__(
        self, *args, modelpath="", threshold=0.0, pooling_operation="max", **kwargs
    ):
        """Class instantiation.
        ...
        logger.debug(
            f"{type(self).__name__} instanciated\n"
    ...
    def process_image(self, image=None, preprocessing=[], tsize=None):
        ...
        if image.shape[:2] > tsize[::-1]:
            image = cv2.resize(image, tsize, interpolation=cv2.INTER_AREA)
        elif image.shape[:2] < tsize[::-1]:
            image = cv2.resize(image, tsize, interpolation=cv2.INTER_CUBIC)
