...
class Dataset(Sequence):
	...
	def __init__(
		self, shuffle=True, dim=(480, 640), color_space='RGB', 
		augmentation=None, channels=3, batch_size=1, class_map=None, 
		preprocess=[]
	):
		self.dim = dim
		self._image_ids = []
...
class OpenEDS(Dataset):
	...
	def load_eyes(self, dataset_dir, subset):
		...
		"""
		# Add classes
		self.add_class("eye", 1, "eye")
		self.add_class("eye", 2, "sclera")
		self.add_class("eye", 3, "iris")
		self.add_class("eye", 4, "pupil")
		
		# Train or validation dataset?
		assert subset in ["train", "val", "test"]
		dataset_dir = os.path.join(dataset_dir, subset)
		
		images = [os.path.join(dataset_dir, 'images', x) for x in sorted(os.listdir(os.path.join(dataset_dir, 'images')))]
		labels = [os.path.join(dataset_dir, 'labels', x) for x in sorted(os.listdir(os.path.join(dataset_dir, 'labels')))]

		# Add images
		for impath, lbpath in zip(images, labels):
			imname = impath.split('/')[-1]
			lb = np.load(lbpath)
			num_ids = np.unique(lb)+1
	...
	def load_mask(self, image_ids):
		...
		for idx, imid in enumerate(image_ids):
			...
			try:
				for lbvalue in np.unique(lb):
					# if lbvalue == 0: _class = np.where(lb == lbvalue, 0, 1)
					# else: _class = np.where(lb == lbvalue, 1, 0)
					bs_mask[idx, ..., lbvalue] = np.where(lb == lbvalue, 1, 0)
			except Exception as ex:
				print(f'Error in image {image_info["id"]}')
				bs_mask[idx, ...] = np.zeros(
					[image_info["height"], image_info["width"], len(num_ids)], 
					dtype=np.float32
				)
