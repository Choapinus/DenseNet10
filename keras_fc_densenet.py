...
def _conv_block(
    ...
    with tf.name_scope(block_prefix):
        ...
        x = BatchNormalization(momentum=bn_momentum, axis=concat_axis, epsilon=1.1e-5)(
            x
        )
        x = Activation("relu")(x)
...
def _transition_down_block(
    x,
    nb_filter,
    bn_momentum,
    weight_decay=1e-4,
    transition_pooling="max",
    block_prefix="TransitionDown",
    data_format="channels_last",
):
    """
...
def _transition_up_block(
    x,
    nb_filters,
    type="deconv",
    weight_decay=1e-4,
    block_prefix="TransitionUp",
    data_format="channels_last",
):
    """
...
def _create_fc_dense_net(
    ...
    with tf.name_scope(name_scope):
        ...
        if data_format not in ["channels_first", "channels_last"]:
            raise ValueError(
                "Invalid data_format: %s. Must be one of [channels_first, channels_last]"
                % data_format
            )
        ...
        if channel not in [1, 3]:
            raise ValueError(
                "Invalid number of channels: %d. Must be one of [1,3]" % channel
            )
        ...
        if type(nb_layers_per_block) is list or type(nb_layers_per_block) is tuple:
            ...
            if len(nb_layers) != (nb_dense_block + 1):
                raise ValueError(
                    "If `nb_layers_per_block` is a list, its length must be "
                    "(`nb_dense_block` + 1)"
                )
        ...
        if row % img_downsize_factor != 0:
            raise ValueError(
                "Invalid image height %d. Image height must be a multiple of "
                "2^nb_dense_block=%d" % (row, img_downsize_factor)
            )
        if col % img_downsize_factor != 0:
            raise ValueError(
                "Invalid image width %d. Image width must be a multiple of "
        ...
        _, nb_filter, concat_list = _dense_block(
            x,
        ...
        for block_idx in range(nb_dense_block):
            ...
            l = concatenate(
                concat_list[1:],
                axis=concat_axis,
                name="Concat_DenseBlock_out_%d" % block_idx,
            )

            t = _transition_up_block(
                l,
                nb_filters=n_filters_keep,
                type=upsampling_type,
                weight_decay=weight_decay,
                data_format=data_format,
                block_prefix="TransitionUp_%i" % block_idx,
            )

            # concatenate the skip connection with the transition block output
            x = concatenate(
                [t, skip_list[block_idx]],
                axis=concat_axis,
                name="Concat_SkipCon_%d" % block_idx,
            )

            # Dont allow the feature map size to grow in upsampling dense blocks
            x_up, nb_filter, concat_list = _dense_block(
                x,
...
def build_FC_DenseNet(
    ...
    if model_version == "fcdn10":
        return build_FC_DenseNet10(
            nb_classes, final_softmax, input_shape, dropout_rate, data_format
        )
    elif model_version == "fcdn56":
        return build_FC_DenseNet56(
            nb_classes, final_softmax, input_shape, dropout_rate, data_format
        )
    elif model_version == "fcdn67":
        return build_FC_DenseNet67(
            nb_classes, final_softmax, input_shape, dropout_rate, data_format
        )
    elif model_version == "fcdn103":
        return build_FC_DenseNet103(
            nb_classes, final_softmax, input_shape, dropout_rate, data_format
        )
    else:
